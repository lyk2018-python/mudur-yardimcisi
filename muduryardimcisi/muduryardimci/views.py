...
def superuser_token(request):
    ...
    get_course_token = Courses.objects.filter(
        course_name=get_course_id).update(
        course_token=token)
    if request.user.is_superuser:
...
def stundent_check(request):
    ...
    get_start_time = Site.objects.get(
        is_active=True, name=main_site_name).course_start
    get_total_morning_date = float(
        Site.objects.get(
            is_active=True,
            name=main_site_name).total_morning_date)
    get_total_afternoon_date = float(
        Site.objects.get(
            is_active=True,
            name=main_site_name).total_afternoon_date)
    ...
    def calucate_time(time):
        try:
            ...
            return ("Fatal Error... ")
        if time == "evening":
            find_time = int(start_hour) + \
                get_total_morning_date + get_total_afternoon_date
        elif time == "afternoon":
    ...
    if time_now >= morning and time_now < afternoon and time_now < evening:
        ...
        if time_now > evening + timeout:
            ...
            check_time = "evening"
    get_course_id = Profile.objects.get(
        user=request.user, is_trainer=False).course_id
...
def AuthToken(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            get_course_id = Profile.objects.get(
                user=request.user, is_trainer=False).course_id
            get_course_token = Courses.objects.get(
                course_name=get_course_id).course_token
            hour_now, min_now = str(strftime("%H:%M", localtime())).split(":")
            min_now = float(min_now) / 100
            hour_now = float(hour_now)
            time_now = hour_now + min_now
            ...
            if token_label == get_course_token and (
                ...
                Profile.objects.filter(
                    ...
                    token_remains=get_token_remains - 1)
                return HttpResponse("<strong><p>Token Error.</strong></html>")
    return render(
        request, 'authlogin.html', {
            'form': form, "token_remains": get_token_remains, })
